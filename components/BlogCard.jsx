...
const BlogCard = ({ imgUrl, date, title, subtitle, sections, index }) => {
    ...
    <motion.div variants={fadeIn('up', 'spring', index * 0.4, 1.5)}>
        <div className='flex md:flex-row flex-col gap-4 hover:scale-105 transition duration-500'>
            <img 
                src={imgUrl}
                alt={title}
            ...
            <div className='w-full flex justify-between items-center'>
                <div className={`p-4 flex-1  ml-2 lg:ml-12 flex flex-col max-w-[650px] gap-4`}>
                    <p className='opacity-60'>{date}</p>
                    <h4 className='font-normal text-[20px] lg:text-[26px] text-palette-1'>{title}</h4>
                    <p className='font-normal lg:text-[20px] text-[14px]'>{subtitle}</p>
